	...
	addr(4)=bdShareVariable(polarizbd%derivStep,'step'    ,opt=true,def=real(0.025,kind=rglu),expect='range(0,1)')
	void=bdCollect('polariz',addr(1:4),bdstart,bdstop,bdcomment,bdaccord,bdseparator,false,0)

	addr(1)=bdShareVariable(densitybd%dtype,'elements'             ,opt=true,def='all',expect='list(charges,orders,all)')
	...
	addr(4)=bdShareVariable(densitybd%derivStep,'derive-step'      ,opt=true,def=real(0.025,kind=rglu),expect='range(0,1)')
	...
	addr(4)=bdShareVariable(coulsonbd%derivStep,'step'      ,opt=true,def=real(0.02,kind=rglu),expect='range(0,1)')
	...
	addr(5)=bdShareVariable(hyperchargesbd%derivaStep,'derive-alpha-step',opt=true,def=real(0.025,kind=rglu),expect='range(0,1)')
	addr(6)=bdShareVariable(hyperchargesbd%derivfStep,'derive-field-step',opt=true,def=real(0.025,kind=rglu),expect='range(0,1)')
	...
	addr(3)=bdShareVariable(fcibd%accuracy,'accuracy'           ,opt=true,def=real(9,kind=rglu),expect='range(1,15)')
	addr(4)=bdShareVariable(fcibd%zeroThreshold,'zero-threshold',opt=true,def=real(12,kind=rglu),expect='range(1,15)')
