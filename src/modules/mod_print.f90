...
!   ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~ CONSTANTS ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~   !
    ...
    real*8   , parameter         :: defsptol=epsilon(defsptol)*10
...
!   ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~   !
    ...
    ufmt='(1X,'//prStrByVal(ucolumns)//'(1X,i'//prStrByVal(indexLen)//',")",'//&
                 prStrByVal(fixedDistance-1)//'X,'//trim(ufmt)//'))'
...
!   ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~   !
    ...
    ueigenfmt='(1X,' //prStrByVal(nmbrRowLen)//'X,4X,'//prStrByVal(ucolumns)//'('//&
                       prStrByVal(valShift)//'X,'//trim(ufmt)//'))'
    ufmt     ='(1X,i'//prStrByVal(nmbrRowLen)// ',4X,'//prStrByVal(ucolumns)//'('//&
                       prStrByVal(valShift)//'X,'//trim(ufmt)//'))'
...
!   ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~   !
    ...
    if (dsparse) then
        ...
        nmbrRowLen=mid(N); nmbrColLen=mid(M)
        ufmt='(1X,i'//prStrByVal(nmbrRowLen)//',1X,i'//prStrByVal(nmbrColLen)//','//&
                      prStrByVal(fixedDistance)//'X,'//trim(ufmt)//')'
        do i = 1,N
        do j = 1,M
    ...
    select case (switch) ! collected to be more readable (despite code redundancy)
        ...
        case ('010') ! NOT ucolumns defined | save order | NOT equal width
            ...
            do ucolumns = N,1,-1
                ...
                enddo
                wdth=sum(lgrid(0,:))+(ucolumns-1)*uinterColWidth
        ...
        do c = 1,lenTrimArray( grid(l,:) )
            ...
            select case (trim(uadjust))
                case ('left')  ; pstr=pstr(1:wdth)//&
                                 hold(grid(l,c))%get()//tpFill(uinterColWidth+ln,uspacer)
                case ('center'); pstr=pstr(1:wdth)//tpFill(ln/2,uspacer)//&
                                 hold(grid(l,c))%get()//tpFill(uinterColWidth+ln/2+mod(ln,2),uspacer)
                case ('right') ; pstr=pstr(1:wdth)//tpFill(ln  ,uspacer)//&
                                 hold(grid(l,c))%get()//tpFill(uinterColWidth,uspacer)
            end select
...
!   ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~   !
    ...
    if (numerate) then
        ...
        !fmt=tpFill(fmt)
        !fmt="('"//umarker(:sta)//"'"//",i"//prStrByVal(numwidth)//"."//prStrByVal(numwidth)//&
        !                           ",1X,'"//umarker(sto+1:len_trim(umarker))//"',A)"
                ...
                if (.NOT.fnddot) then
                    ...
                    select case (keycode(4:6))
                        ...
                        case ('101')
                            ...
                            ufmt=prStrByVal(zBeforeDot)//'PE'//prStrByVal(vLen)//'.'//&
                                 prStrByVal(fixedPresc)//'E'//prStrByVal(zBeforeEnd)
