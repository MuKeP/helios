	...
	use glob     , only: rglu,iglu,true,void,gluZero,gluUnity,false
	use hdb      , only: in,eu,lrg
	use hdb      , only: generalbd,systembd,geometrybd,statesbd,polarizbd,diisbd
	...
	addr(3)=bdShareVariable(statesbd%hftype ,'excited-states-method',opt=true,def='cis',expect='list(cis,rpa)')
	void=bdCollect('states',addr(1:3),bdstart,bdstop,bdcomment,bdaccord,bdseparator,false,0)
	...
	addr(1)=bdShareVariable(lrbd%guess         ,'guess'            ,opt=true,def='cis',expect='list(unt,cis,rpa)')
	addr(2)=bdShareVariable(lrbd%guessThreshold,'guess-threshold'  ,opt=true,def=real(0.1d0,kind=rglu),expect='range(0,1)')
	addr(3)=bdShareVariable(lrbd%maxiters      ,'max-iterations'   ,opt=true,def=10000)
	addr(4)=bdShareVariable(lrbd%accuracy      ,'accuracy'         ,opt=true,def=real(6,kind=rglu),expect='range(1,15)')
	addr(5)=bdShareVariable(lrbd%iterStep      ,'iter-step'        ,opt=true,def=real(0.03d0,kind=rglu),expect='range(0,1)')
	addr(6)=bdShareVariable(lrbd%orthogonalize ,'orthogonalization',opt=true,def=true)
	void=bdCollect('linear-responce',addr(1:6),bdstart,bdstop,bdcomment,bdaccord,bdseparator,false,0)
