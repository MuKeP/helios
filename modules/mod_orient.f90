...
!   ~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~   !
    ...
    x1=oX(jj)-oX(ii); y1=oY(jj)-oY(ii); z1=oZ(jj)-oZ(ii)
    x2=oX(jj)-oX(kk); y2=oY(jj)-oY(kk); z2=oZ(jj)-oZ(kk)

    zna =sqrt( (x1*x1+y1*y1+z1*z1) * (x2*x2+y2*y2+z2*z2) )
    chis=x1*x2+y1*y2+z1*z2
    if (zna.LT.1e-15_rglu) then
        zna=gluUnity
    endif
    cos_ang=chis/zna
