	...
	real   (kind=rglu), parameter  :: derivThreshold(4)=[1D-3,1D-2,1D-1,1D0]
	...
	Nel=mol%nEls; Nat=mol%uniqueAtoms; scale=uchGet(polarizbd%scales); Np=polarizbd%nPoints
	...
	if (Np.GE.5) void=lsmInterpolation( len_trim(uchGet(polarizbd%scales)) )
