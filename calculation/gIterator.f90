	...
	use glob     , only: rglu,iglu,lglu,timeControl,gluCompare
	...
	real   (kind=rglu), parameter  :: feelDivergence=1000,frequency=-5,notRearly=20,&
	                                  feelStagnation=60,stagnationIterationThreshold=100
	...
	do
		...
		if (iteration.EQ.1) then
			if (avgTimePerIter.GT.frequency) then
				printRate=1
			else
				if (avgTimePerIter.LT.10**6*gluCompare) then
					printRate=notRearly
				else
					printRate=int(frequency/avgTimePerIter)
				endif
			endif
		...
		if (iteration.GT.stagnationIterationThreshold) then
		if (successPercent.LE.feelStagnation) then
