	...
	integer(kind=iglu) :: ab(2),t,mm,nn,i,a,j,b,k,c,l,d
...
!								+t3(i,j,k,a,b,c)&
		...
		sum=0
		k=i; l=j; sum=sum+R(i,k,j,l)*(real(0.5,rglu)*t2(k,l,a,b)+t1(k,a)*t1(l,b))
		k=j; l=i; sum=sum+R(i,k,j,l)*(real(0.5,rglu)*t2(k,l,a,b)+t1(k,a)*t1(l,b))
		c=a; d=b; sum=sum+R(a,c,b,d)*(real(0.5,rglu)*t2(i,j,c,d)+t1(i,c)*t1(j,d))
		c=b; d=a; sum=sum+R(a,c,b,d)*(real(0.5,rglu)*t2(i,j,c,d)+t1(i,c)*t1(j,d))
		rez=rez+sum
		...
		do k = 1,Nel
			...
			do t = 1,2
				...
				sum=sum+R(b,c,k,d)*(&
										...
										+real(0.5,rglu)*(t1(k,a)*t2(i,j,c,d)&
	!									+t3(i,j,k,a,c,d)&
										))&
										)
			enddo
			...
			do t = 1,2
				...
				sum=sum-R(a,c,k,d)*(&
										...
										+real(0.5,rglu)*(t1(k,b)*t2(i,j,c,d)&
	!									+t3(i,j,k,b,c,d)&
										))&
										)
			enddo
	...
	!									          +t3(i,k,l,a,b,c)&
		...
		do l = 1,Nel
			...
			do t = 1,2
				...
				+real(0.25,rglu)*t2(i,j,c,d)*t2(k,l,a,b)&
